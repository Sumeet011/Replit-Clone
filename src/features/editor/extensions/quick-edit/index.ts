...
const quickEditTooltipField = (
  ...
  create(state) {
    return createQuickEditTooltip(state, getUserId);
  },

  update(tooltips, transaction) {
    if (transaction.docChanged || transaction.selection) {
      return createQuickEditTooltip(transaction.state, getUserId);
    }
    for (const effect of transaction.effects) {
      if (effect.is(showQuickEditEffect)) {
        return createQuickEditTooltip(transaction.state, getUserId);
...
export const quickEdit = (
  fileName: string,
  getUserId: () => string | null | undefined = () => undefined,
) => [
  quickEditState,
  quickEditTooltipField(getUserId),
  quickEditKeymap,
